    ...
    return (
        <div className="flex flex-col h-full bg-transparent">
            <div className="px-6 py-5 border-b border-neutral-200 flex flex-col sm:flex-row sm:items-center justify-between bg-neutral-50 gap-4">
                <div>
                    <h3 className="text-[13px] font-bold text-neutral-900 uppercase tracking-wider">Categorias GERAIS</h3>
                    <p className="text-xs text-neutral-500 mt-1">Defina as secções gerais (ex: Geral, VIP, Mesa).</p>
                </div>
                <button
                    onClick={startCreate}
                    className="inline-flex items-center justify-center gap-2 bg-violet-600 text-white hover:bg-violet-700 px-4 py-2.5 rounded-2xl text-[13px] font-bold shadow-md transition-all hover:-translate-y-0.5 active:scale-95 shrink-0"
                >
                ...
                <div className="p-6 sm:p-8 border-b border-neutral-200 bg-neutral-50 relative">
                    ...
                    <form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label className="block text-[11px] font-bold text-neutral-500 uppercase tracking-[0.2em] mb-2 ml-1">Nome do Tipo *</label>
                                <input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} disabled={!!editingTypeId && !canEdit} placeholder="Ex: Entrada VIP" className="w-full h-12 rounded-2xl border border-neutral-200 bg-neutral-50 text-neutral-900 placeholder:text-neutral-400 px-5 focus:outline-none focus:ring-1 focus:ring-violet-200 transition-all shadow-inner text-[14px] disabled:opacity-50" />
                            </div>
                            <div>
                                <label className="block text-[11px] font-bold text-neutral-500 uppercase tracking-[0.2em] mb-2 ml-1">Limite por Pessoa</label>
                                <input type="number" min="1" value={formData.perUserLimit} onChange={e => setFormData({ ...formData, perUserLimit: e.target.value })} disabled={!!editingTypeId && !canEdit} placeholder="Ex: 4 (Opcional)" className="w-full h-12 rounded-2xl border border-neutral-200 bg-neutral-50 text-neutral-900 placeholder:text-neutral-400 px-5 focus:outline-none focus:ring-1 focus:ring-violet-200 transition-all shadow-inner text-[14px] disabled:opacity-50" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-[11px] font-bold text-neutral-500 uppercase tracking-[0.2em] mb-2 ml-1">Descrição</label>
                            <input value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} disabled={!!editingTypeId && !canEdit} placeholder="Inclui 2 bebidas e acesso prioritário" className="w-full h-12 rounded-2xl border border-neutral-200 bg-neutral-50 text-neutral-900 placeholder:text-neutral-400 px-5 focus:outline-none focus:ring-1 focus:ring-violet-200 transition-all shadow-inner text-[14px] disabled:opacity-50" />
                        </div>
                        {editingTypeId && canEdit && (
                            <div>
                                ...
                                <select value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value as "ACTIVE" | "PAUSED" })} className="w-full h-12 rounded-2xl border border-neutral-200 bg-neutral-50 text-neutral-900 px-5 text-[14px]">
                                    <option value="ACTIVE" className="bg-background">Ativo</option>
                                    <option value="PAUSED" className="bg-background">Pausado</option>
                                </select>
                            </div>
                        )}
                        <label className="flex items-center gap-4 p-4 border border-neutral-200 bg-neutral-50 rounded-2xl cursor-pointer hover:bg-neutral-100 transition-colors mt-2">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-xs text-neutral-500 font-medium mt-0.5">Marque apenas se este tipo precisar que o cliente escolha um lugar no mapa da sala.</span>
                            </div>
                        </label>
            ...
            {types.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 text-center">
                    <div className="w-16 h-16 bg-neutral-50 border border-neutral-200 rounded-3xl flex items-center justify-center mb-5 text-neutral-400 shadow-inner">
                        <Ticket className="h-7 w-7" />
                    </div>
                    <h4 className="text-[15px] font-bold text-neutral-900">Sem Tipos de Bilhete</h4>
                    <p className="text-[13px] text-neutral-500 mt-2 max-w-sm font-medium leading-relaxed">Comece por criar uma categoria geral como &quot;Bilhete Simples&quot; ou &quot;Mesa VIP&quot; para associar os futuros Lotes de venda a essa categoria.</p>
                </div>
            ) : (
                <div className="divide-y divide-neutral-200 bg-transparent flex-1">
                    {types.map(t => {
                        ...
                        return (
                            <div key={t.id} className={`p-4 sm:p-5 px-6 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-neutral-50 transition-colors gap-4 ${isEditing ? "bg-emerald-500/5" : ""}`}>
                                <div>
                                    <h4 className="text-[14px] font-bold text-neutral-900 flex items-center gap-3 flex-wrap">
                                        {t.name}
                                    ...
                                    <p className="text-[13px] text-neutral-500 mt-1 font-medium">{t.description || "Sem descrição especial."}</p>
                                </div>
                                <div className="flex items-center gap-3 shrink-0">
                                    <div className="flex items-center gap-5 text-[12px] font-bold text-neutral-500 uppercase tracking-wider bg-neutral-50 px-4 py-2 rounded-xl border border-neutral-200">
                                    ...
                                    {canEdit && (
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold border border-neutral-200 text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 transition-all"
                                            >
