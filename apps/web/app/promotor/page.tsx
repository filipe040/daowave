        ...
        >
            {loading && (
                <div className="space-y-10">
                    ...
                    <div className="h-[400px] bg-white/5 rounded-[32px] border border-white/10 animate-pulse" />
                </div>
            )}
            ...
            {!loading && !error && !stats && (
                <EmptyState
                    icon={Calendar}
                    title="Bem-vindo ao LivePass"
                    description="Parece que ainda não tem uma organização ou eventos. Comece por criar o seu primeiro evento."
                    action={
                        <Link
                            href="/promotor/events/new"
                            className="inline-flex items-center px-8 py-4 rounded-2xl text-base font-bold bg-[#00a0e3] text-white hover:bg-[#0090cc] transition-all shadow-md"
                        >
            ...
            {!loading && !error && stats && (
                <div className="space-y-12 pb-20">
                    ...
                    {/* Chart Section */}
                    <div className="rounded-3xl border border-white/10 bg-[#14141f] shadow-md p-8 sm:p-10">
                        <div className="flex items-center justify-between mb-10">
                            <div>
                                <h3 className="text-xl font-bold text-white tracking-tight">Histórico de Vendas</h3>
                                <p className="text-sm text-zinc-500 mt-1">Volume de faturação nos últimos 30 dias</p>
                            </div>
                            <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl border border-white/10 text-[12px] font-bold text-zinc-400">
                        ...
                        <div className="flex items-end justify-between gap-1 sm:gap-2 h-[240px] px-2">
                            {stats.chart.map((d, i) => (
                                <div key={i} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                                    {/* Tooltip */}
                                    <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-[#14141f] text-black text-[11px] font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all pointer-events-none whitespace-nowrap shadow-xl z-10 scale-90 group-hover:scale-100 origin-bottom">
                                        {fmt(d.revenue * 100)}
                                    </div>

                                    {/* Bar */}
                                    <div
                                        className="w-full max-w-[12px] rounded-full bg-neutral-200 group-hover:bg-[#00a0e3] transition-all duration-500 relative overflow-hidden"
                                    ...
                                    {(i % 5 === 0 || i === stats.chart.length - 1) && (
                                        <div className="mt-4 text-[9px] font-bold text-zinc-500 uppercase tracking-tighter sm:tracking-normal">
                    ...
                    <div className="grid gap-10 lg:grid-cols-3">
                        <div className="lg:col-span-2 space-y-6">
                            <div className="flex items-center justify-between px-2">
                                ...
                                <Link href="/promotor/events" className="text-[13px] font-bold text-zinc-500 hover:text-white transition-colors flex items-center gap-2">
                            ...
                            <div className="space-y-3">
                                {stats.recentEvents.map((event) => (
                                    <div key={event.id} className="rounded-3xl border border-white/10 bg-[#14141f] shadow-md p-5 sm:p-6 hover:bg-white/10 transition-all group flex items-center justify-between gap-4">
                                        <div className="flex items-center gap-5 min-w-0">
                                            <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center shrink-0">
                                                <span className="text-[10px] font-black text-zinc-500 uppercase leading-none">{format(new Date(event.startAt), "MMM", { locale: pt })}</span>
                                                <span className="text-xl font-black text-white">{format(new Date(event.startAt), "dd")}</span>
                                            </div>
                                            <div className="min-w-0">
                                                <h4 className="font-bold text-white truncate text-base group-hover:text-emerald-600 transition-colors uppercase tracking-tight">{event.title}</h4>
                                                <p className="text-[13px] text-zinc-500 mt-0.5">{event.city} • <span className="font-bold text-zinc-500">{event._count.tickets} bilhetes</span></p>
                                            </div>
                                        </div>

                                        <div className="flex items-center gap-2">
                                            <span className={`hidden sm:inline-flex px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase border ${event.status === 'PUBLISHED' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600' : 'bg-white/5 border-white/10 text-zinc-500'
                                                }`}>
                                ...
                                {stats.recentEvents.length === 0 && (
                                    <div className="bg-white/5 border border-dashed border-white/10 rounded-[24px] p-10 text-center">
                                        <p className="text-zinc-400 font-medium">Nenhum evento criado recentemente.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {[
                                    ...
                                    <Link key={item.href} href={item.href} className="rounded-3xl border border-white/10 bg-[#14141f] shadow-md p-6 hover:bg-white/10 hover:border-neutral-300 transition-all group shadow-xl flex items-center gap-4">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-[12px] text-zinc-500 mt-0.5 leading-tight">{item.desc}</p>
                                        </div>
                                    </Link>
                                ))}
                            </div>
