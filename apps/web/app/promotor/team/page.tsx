        ...
        >
            {loading && (
                <div className="bg-[#14141f] rounded-2xl border border-white/10 shadow-md overflow-hidden">
                    {Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} className="px-6 py-4 border-b border-white/10 last:border-0">
                        ...
                        <thead>
                            <tr className="border-b border-white/10">
                                {["Membro", "Organização", "Função", "Desde", ...(canRemoveMembers ? ["Ações"] : [])].map((h) => (
                                    <th key={h} className="px-6 py-4 text-left text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">
                                        {h}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {members.map((m) => (
                                <tr key={m.id} className="hover:bg-white/5 transition-colors group">
                                    <td className="px-6 py-5">
                                        <div className="font-bold text-white group-hover:text-emerald-600 transition-colors uppercase tracking-tight">{m.user.name ?? m.user.email.split('@')[0]}</div>
                                        <div className="text-[11px] text-zinc-500 font-medium">{m.user.email}</div>
                                    </td>
                                    <td className="px-6 py-5 text-zinc-500 font-bold uppercase tracking-tight text-xs">{m.organization.name}</td>
                                    <td className="px-6 py-5">
            ...
            {showInviteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
                    ...
                    <div className="relative w-full max-w-lg bg-[#0c0c12] border border-white/10 rounded-[32px] shadow-2xl animate-in zoom-in-95 duration-200 p-8 sm:p-10">
                        <button
                        ...
                        <div className="flex flex-col items-center text-center space-y-4 mb-10">
                            ...
                            <div>
                                ...
                                <p className="text-zinc-500 text-[13px] font-medium mt-1">Envie um convite para colaborar na sua organização.</p>
                            </div>
                        </div>

                        <form onSubmit={handleInvite} className="space-y-6">
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500 block mb-2.5 ml-1">Endereço de Email</label>
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-6 text-white placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-neutral-200 transition-all font-medium"
                                    />
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {inviteRoles.map(role => (
                                        <button
                                            ...
                                            className={`flex items-start gap-4 p-4 rounded-2xl border text-left transition-all ${inviteRole === role.id ? 'bg-[#00a0e3]/10 border-[#00a0e3]/40' : 'bg-white/5 border-white/10 hover:border-neutral-300'}`}
                                        >
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${inviteRole === role.id ? 'bg-[#00a0e3]' : 'bg-neutral-100'}`}>
                                                <ShieldCheck className={`w-5 h-5 ${inviteRole === role.id ? 'text-white' : 'text-zinc-500'}`} />
                                            </div>
                                            <div>
                                                <div className="text-xs font-black text-white uppercase tracking-widest">{role.label}</div>
                                                <div className="text-[11px] text-zinc-500 mt-1 font-medium leading-relaxed">{role.desc}</div>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                            ...
                            <button
                                type="submit"
                                disabled={inviting}
                                className="w-full py-5 rounded-2xl bg-[#00a0e3] text-white font-black uppercase tracking-widest text-xs hover:bg-[#0090cc] disabled:opacity-50 transition-all shadow-md"
                            >
