...
export default function AccountFavorites({
  ...
  if (initialEvents.length === 0) {
    return (
      <div className="rounded-3xl border border-neutral-200 bg-white p-10 sm:p-14 text-center">
        <Heart className="h-14 w-14 text-neutral-300 mx-auto mb-4" strokeWidth={1.5} />
        <h1 className="text-xl font-bold text-neutral-900">Sem favoritos</h1>
        <p className="mt-2 text-sm text-neutral-500 max-w-md mx-auto">
          Guarda eventos que te interessam clicando no coração nas páginas de eventos.
        </p>
        <Link
          href="/events"
          className="inline-flex mt-6 items-center gap-2 px-5 py-2.5 rounded-xl bg-violet-600 text-white text-sm font-bold hover:bg-violet-700"
        >
          Explorar eventos
        </Link>
      </div>
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-black text-neutral-900">Favoritos</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        {initialEvents.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="relative group overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-md hover:shadow-lg transition-all"
            >
              ...
              <Link href={`/events/${event.slug}`} className="block">
                ...
                <div className="p-5">
                  <h2 className="text-lg font-bold text-neutral-900 line-clamp-2 pr-10">{event.title}</h2>
                  ...
                  <div className="mt-4 pt-4 border-t border-neutral-100 flex items-center justify-between">
                    {minPrice != null ? (
                      <span className="text-sm font-bold text-violet-700">Desde {formatPrice(minPrice)}</span>
                    ) : (
                      <span className="text-sm text-neutral-500">Consultar preço</span>
                    )}
